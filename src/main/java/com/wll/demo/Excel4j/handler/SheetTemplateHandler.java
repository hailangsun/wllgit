...
package com.wll.demo.Excel4j.handler;
...
public class SheetTemplateHandler {
        ...
        try {
            ...
            try {
                sheetTemplate.workbook = WorkbookFactory.create(
                        com.wll.demo.Excel4j.handler.SheetTemplateHandler.class.getResourceAsStream(templatePath)
                );
    ...
    private static void initModuleConfig(SheetTemplate template) {
        ...
        for (Row row : template.sheet) {
            for (Cell c : row) {
                ...
                // 寻找序号列
                if (com.wll.demo.Excel4j.handler.HandlerConstant.SERIAL_NUMBER.equals(str)) {
                    template.serialNumberColumnIndex = c.getColumnIndex();
                }
                // 寻找数据列
                if (com.wll.demo.Excel4j.handler.HandlerConstant.DATA_INIT_INDEX.equals(str)) {
    ...
    private static void initStyles(SheetTemplate template, Cell cell, String moduleContext) {
        ...
        if (com.wll.demo.Excel4j.handler.HandlerConstant.DEFAULT_STYLE.equals(moduleContext)) {
        ...
        if (com.wll.demo.Excel4j.handler.HandlerConstant.APPOINT_LINE_STYLE.equals(moduleContext)) {
            template.appointLineStyle.put(cell.getRowIndex(), cell.getCellStyle());
            clearCell(cell);
        }
        if (com.wll.demo.Excel4j.handler.HandlerConstant.SINGLE_LINE_STYLE.equals(moduleContext)) {
            template.singleLineStyle = cell.getCellStyle();
            clearCell(cell);
        }
        if (com.wll.demo.Excel4j.handler.HandlerConstant.DOUBLE_LINE_STYLE.equals(moduleContext)) {
